...
#                 self.labels, self.sp_data = pickle.load(f)
                ...
                for i in range (way):
                    tlabelway=[]
                    dataway=[]
                    tlabel.append(tlabelway)
                    tdata.append(dataway)
                rlabels, rsp_data = pickle.load(f)
                
                pair=list(zip(rlabels, rsp_data))
                for sample in (pair):
                    for w in range(way):
                        if (sample[0] == ways[w]): 
                            tlabel[w].append(ways[w])
                            tdata[w].append(sample[1])
#                     elif (sample[0] == ways[1]): 
#                         tlabels2.append(sample[0])
#                         tsp_data2.append(sample[1])
#                     elif (sample[0] == ways[2]): 
#                         tlabels3.append(sample[0])
#                         tsp_data3.append(sample[1])
#                 print(tlabel)
                for w in range(way):
                    tlabel[w]=[ways[w]] * shot
            
                if(split=='test'):
                    for w in range(way):
                        tdata[w]=tdata[w][it*shot:(it+1)*shot]
#                         tsp_data1=tsp_data1[it*5:(it+1)*5]
#                         tsp_data2=tsp_data2[it*5:(it+1)*5]
#                         tsp_data3=tsp_data3[it*5:(it+1)*5]    
                else:
                    for w in range(way):
                        tdata[w]=tdata[w][(len(tdata[w])-(it+1)*shot):(len(tdata[w])-it*shot)]
#                     tsp_data1=tsp_data1[len(tsp_data1)-(it+1)*5:len(tsp_data1)-it*5]
#                     tsp_data2=tsp_data2[len(tsp_data2)-(it+1)*5:len(tsp_data2)-it*5]
#                     tsp_data3=tsp_data3[len(tsp_data3)-(it+1)*5:len(tsp_data3)-it*5] 
                zlabels=tlabel[0]
                zsp_data=tdata[0]
                for z in range (1,way):
                    zlabels=zlabels+tlabel[z]
                    zsp_data=zsp_data+tdata[z]
#                 zlabels=tlabels1+tlabels2+tlabels3
#                 zsp_data=tsp_data1+tsp_data2+tsp_data3
                
                mapIndexPosition = list(zip(zlabels, zsp_data))
                random.shuffle(mapIndexPosition)
                self.labels, self.sp_data = zip(*mapIndexPosition)
...
#             print('take a look index',index)
#             print('take a look sample',sample)
            mean_px, coord = sample[:2]
#             print('take a look mean_px',mean_px)
#             print('take a look coord',coord)
            try:
...
class SuperPixDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name,testset,trainset,it,way,task,shot,num_val=0):
        """
        ...
        t_data = time.time()
        self.name = name
        ways=random.sample(trainset,way)
        ...
        self.test = SuperPixDGL("./data/superpixels", dataset=self.name, split='test', select=testset,it=it,way=way,task=task,
                            shot=shot,
                            ways=ways,
                            use_mean_px=use_mean_px, 
                            use_coord=use_coord)
  
        self.train_ = SuperPixDGL("./data/superpixels", dataset=self.name, split='train',select=trainset,it=it,way=way,
                             task=task,
                             shot=shot,
                             ways=ways,
                             use_mean_px=use_mean_px, 
...
#         traind=[]
# #         train=[]
#         with open('./data/superpixels/mnist_75sp_train.pkl', 'rb') as f:
#             self.train = pickle.load(f)
# #             rlabels, rsp_data = pickle.load(f)
# #             pair=list(zip(rlabels, rsp_data))
# #             for sample in (pair):
# #                     if (sample[0] in testset): 
# #                         testl.append(sample[0])
# #                         testd.append(sample[1])
# #                     elif(sample[0] in trainset):
# #                         trainl.append(sample[0])
# #                         traind.append(sample[1])
                           
#         with open('./data/superpixels/mnist_75sp_test.pkl', 'rb') as f:
#             rlabels, rsp_data = pickle.load(f)
#             pair=list(zip(rlabels, rsp_data))
#             for sample in (pair):
#                     if (sample[0] in testset): 
